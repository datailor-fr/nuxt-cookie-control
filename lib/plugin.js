...
export default(context, inject) =>{
  ...
  let methods = {
    ...
    setBlockedIframes: (content) =>{
      ...
      c = c.replace(/&lt;/g, '<');
      c = c.replace(/&gt;/g, '>');
      if(context.app.$cookiesControl.enabled.filter(c =>{return c.name === 'functional'}).length === 0){
        c = c.replace(/<iframe/g, `<div class='cookieControl__BlockedIframe '`);
        c = c.replace(/<\/iframe/g, `<p>${context.app.$cookiesControl.text.blockedIframe !== undefined ? context.app.$cookiesControl.text.blockedIframe : ''} <a href='#' onclick='event.preventDefault(); $${cookies.globalName}.$cookiesControl.modal = true'>${context.app.$cookiesControl.text.here !== undefined ? context.app.$cookiesControl.text.here: ''}</a></p></div`); 
      }
